...
public class PickingUpDamageUp {
    Ship player;
    private int playerX;
    private int playerY;
    private int boxX;
    ...
    public PickingUpDamageUp(Ship player){
        this.player = player;
    }

    private void checkAll(){
        if(SpawnedPowerUps.damageUpArray.size() > 0){
            for(int i = SpawnedPowerUps.damageUpArray.size() - 1; i >= 0; i--){
                DamageUp picked = SpawnedPowerUps.damageUpArray.get(i);
    ...
    private void setBoxBorders(DamageUp picked){
    ...
    private void makeAction(DamageUp picked){
        picked.getAction().addDamage();
    }

    private void removeObjectFromArray(DamageUp picked){
        SpawnedPowerUps.damageUpArray.remove(picked);
    }
